<template>
  <transition name="fade" mode="out-in">
    <div v-if="loaded" id="quarantaene" :class="classValue">
      <div v-if="$route.path != '/'" id="rotate-screen-message">
...
export default {
  ...
  computed: {
    ...
    showMenu() {
      return (
        this.$store.state.questions.firstQuestionAnswered &&
        (this.$route.name === 'mainfloor' ||
          this.$route.name === 'bathroom' ||
          this.$route.name === 'bar' ||
          this.$route.name === 'bathroom-bathroom' ||
          this.$route.name === 'imprint' ||
          this.$route.name === 'cloakroom')
      );
    },
    classValue() {
